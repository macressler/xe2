...
(defparameter *psi-samples* '("blurp" "blurp2"))

(defparameter *psi-alt-samples* '("bleep" "bloup"))
...
(define-method paint brick (color)
  (setf <color> color)
  (let ((resource (getf *brick-tiles* color)))
    (assert (stringp resource))
    (setf <tile> resource)))
...
(defparameter *room-height* (truncate (/ (- 600 20) 16)))
(defparameter *room-width* (truncate (/ 800 16)))
