...
(define-method do-collision yasichi (object)
  (labels ((do-box (image)
    ...
    (send-queue self :add-overlay :viewport #'do-box))
...
(define-method light-square yasichi (r c)
  (labels ((do-square (image)
    ...
    (send-queue self :add-overlay :viewport #'do-square)))
...
(define-method alarm monitor ()
  ...
  (labels ((do-circle (image)
    ...
    (send-queue self :add-overlay :viewport #'do-circle))
...
(define-method step puckup (stepper)
  (when (>is-player stepper)
    (when (field-value :puck stepper)
      (send-queue self :narrateln :narrator "You wasted your puck.")
	...
	(if (>probe self dir)
	    (progn (>move self dir :ignoring-obstacles)
		   (let ((piece <behind>)
			 ...
			 next-r next-c)
		     (loop while piece
			   do (send-queue self :move piece (direction-to (setf next-r (field-value :row piece))
...
(define-method find-target muon-particle ()
  (let ((target (>category-in-direction-p *world* 
	...
	(progn
	  (send-queue self :move self <direction>)
	  (send-queue self :expend-default-action-points self)
	  (send-queue self :push target <direction>)
	  (send-queue self :damage target <attack-power>)
	  (send-queue self :die self))
	(multiple-value-bind (r c) 
	    (step-in-direction <row> <column> <direction>)
...
(define-method run oscillator ()
  (with-fields (row column direction) self
